...
public class ChatClient extends Thread{
    ...
    public static void parseData(String data){
		...
		if(json.getInteger("type") == 0){//连接成功
			...
			RotMain.receivePrivateMessages(json.toJSONString());
		}else if(json.getInteger("type") == 2){//群聊消息
			RotMain.receiveGroupMessages(json.toJSONString());
		}else if(json.getInteger("type") == 3){//事件消息
			RotMain.receiveEventMessages(json.toJSONString());
		}else if(json.getInteger("type") == 4){//查询返回
			RotMain.selectResult(json.toJSONString());
		}
